...
public:
	...
	static void ClantagChanger()
	{
		...
		{
			static int Cycle = 0;
			std::vector< string > Strings = { string( "FACELESS" ), string( "FACELESS" ), string( "WANA HVH?" ), string( "WANA HVH?.xyz" ), string( "SLAP.TEAM" ) };
...
/*	static void ThirdPerson( void )
	...
	*/
	...
	static bool Shooting()
	{
		...
		else if (Hacks.LocalWeapon->IsMiscWeapon())
		{
			if (Hacks.CurrentCmd->buttons & IN_ATTACK || Hacks.CurrentCmd->buttons & IN_ATTACK2)
				return true;
		}
	...
	static bool LineGoesThroughSmoke(Vector pos1, Vector pos2) throw()
	{
		...
		//OffsetLineGoes = Utils.PFindPattern("client.dll",			 "55 8B  EC  83  EC  08	8B  15  ?   ?   ?   ?     0F 57 C0");
