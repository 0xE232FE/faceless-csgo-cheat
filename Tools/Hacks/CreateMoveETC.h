...
public:
	...
	static void CleanUp(Vector OrigAng)
	{
		...
		if (Settings.GetSetting(Tab_Ragebot, AntiAim_Yaw) == 3)
		{
			if (Aimbot.Target != Vector(0, 0, 0))
			{
				Misc::CalcAngle(Hacks.LocalPlayer->GetVecOrigin(), Aimbot.Target + Hacks.LocalPlayer->GetVecOrigin(), Hacks.LastAngles);
				if (Hacks.LocalPlayer->GetVecVelocity().Length2D() > 1 && Hacks.LocalPlayer->GetFlags() & FL_ONGROUND)
					Hacks.LastAngles.y += 180;
				Hacks.LastAngles.x = Hacks.CurrentCmd->viewangles.x;
			}
			else {
				Hacks.LastAngles.x = Hacks.CurrentCmd->viewangles.x;
				if(Hacks.LocalPlayer->GetVecVelocity().Length2D() > 1 && Hacks.LocalPlayer->GetFlags() & FL_ONGROUND)
